...
UVSphereGenerator::UVSphereGenerator(float radius, int slices, int stacks, const glm::vec3& origin, const glm::vec3& color)
    : MeshGenerator(origin, color), radius_(radius) {
    this->ConstructorHelper_(slices, stacks);
}

std::shared_ptr<IMeshInstance> UVSphereGenerator::GenerateMesh() {
    ...
    auto temp = std::make_shared<MeshInstance>(
        positions,
...
void UVSphereGenerator::ConstructorHelper_(int slices, int stacks) {
    if (slices < 3) {
