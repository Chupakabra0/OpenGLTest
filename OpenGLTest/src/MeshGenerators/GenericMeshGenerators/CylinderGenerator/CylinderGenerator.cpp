...
CylinderGenerator::CylinderGenerator(float height, float radius, int segments, int slices, const glm::vec3& circleOrigin, const glm::vec3& color)
    : MeshGenerator(circleOrigin, color), height_(height), radius_(radius), segments_(segments) {
    this->ConstructorHelper_(slices);
}

std::shared_ptr<IMeshInstance> CylinderGenerator::GenerateMesh() {
...
void CylinderGenerator::ConstructorHelper_(int slices) {
    if (slices < 1) {
